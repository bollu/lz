...
extern "C" object * lean_ir_emit_mlir(object * env, object * mod_name);

string_ref emit_mlir(environment const & env, name const & mod_name) {
    object * r = lean_ir_emit_mlir(env.to_obj_arg(), mod_name.to_obj_arg());
    string_ref s(cnstr_get(r, 0), true);
    if (cnstr_tag(r) == 0) {
        dec_ref(r);
        throw exception(s.to_std_string());
