...
@[extern c "lean_float_add"]  constant Float.add : Float → Float → Float
-- @[extern c inline "#1 - #2"]  constant Float.sub : Float → Float → Float
@[extern c "lean_float_sub"]  constant Float.sub : Float → Float → Float
-- @[extern c inline "#1 * #2"]  constant Float.mul : Float → Float → Float
@[extern c "lean_float_mul"]  constant Float.mul : Float → Float → Float
-- @[extern c inline "#1 / #2"]  constant Float.div : Float → Float → Float
@[extern c "lean_float_div"]  constant Float.div : Float → Float → Float
-- @[extern c inline "(- #1)"]   constant Float.neg : Float → Float
@[extern c  "lean_float_negate"]   constant Float.neg : Float → Float
...
@[extern c "lean_float_lt"]   constant Float.decLt (a b : Float) : Decidable (a < b) :=
  match a, b with
  | ⟨a⟩, ⟨b⟩ => floatSpec.decLt a b

-- @[extern c inline "#1 <= #2"] constant Float.decLe (a b : Float) : Decidable (a ≤ b) :=
@[extern c "lean_float_le"] constant Float.decLe (a b : Float) : Decidable (a ≤ b) :=
  match a, b with
...
@[extern c "lean_float_to_uint8"] constant Float.toUInt8 : Float → UInt8
@[extern c "lean_float_to_uint16"] constant Float.toUInt16 : Float → UInt16
@[extern c "lean_float_to_uint32"] constant Float.toUInt32 : Float → UInt32
@[extern c "lean_float_to_uint64"] constant Float.toUInt64 : Float → UInt64
@[extern c "lean_float_to_usize"] constant Float.toUSize : Float → USize
