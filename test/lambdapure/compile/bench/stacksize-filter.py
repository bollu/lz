...
PROBLEM_SIZES=[2**i for i in range(2, 20)]
...
def run_data():
    ...
    for psz in PROBLEM_SIZES:
        datum = {"problem_size": str(psz) }
        ...
        for _ in range(ARGS.nruns):
          os_system_synch("rm massif.out")
          out = sh(f"valgrind --massif-out-file=massif.out --stacks=yes --tool=massif ./exe-ref.out {psz}");
        ...
        for _ in range(ARGS.nruns):
           os_system_synch("rm massif.out")
           out = sh(f"valgrind --massif-out-file=massif.out --stacks=yes --tool=massif ./exe-mlir.out {psz}");
           massif = msparser.parse_file('massif.out')
...
def plot():
  ...
  for i, data in enumerate(datapoints):
      ...
      theirs.append(np.log(np.median([np.max(run) for run in data["theirs-perf"]])))
      ours.append(np.log(np.median([np.max(run) for run in data["ours-perf"]])))
  ...
  fig, ax = plt.subplots()
  rects1 = ax.bar(x - width/2, theirs, width, label='theirs', color = light_blue)
  rects2 = ax.bar(x + width/2, ours, width, label='ours', color = dark_blue)
  ...
  ax.set_ylabel('Stack size on filter.lean for different problem sizes', rotation='horizontal', position = (1, 1.1),
