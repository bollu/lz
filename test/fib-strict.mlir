...
module {
  ...
  lz.func @fibstrict (%i: !lz.value) -> !lz.value {
      %retval = lz.caseint  %i
      [@default -> { ^entry: // todo: remove this defult
        %fib_rec = lz.ref (@fibstrict):!lz.fn<(!lz.value) -> !lz.value>
        %lit_one = lz.make_i64(1)
        %i_minus_one_v = lz.primop_sub(%i, %lit_one)

        %fib_i_minus_one_t = lz.ap(%fib_rec: !lz.fn<(!lz.value)->  !lz.value>, %i_minus_one_v)
        %fib_i_minus_one_v = lz.force(%fib_i_minus_one_t):!lz.value

        %lit_two = lz.make_i64(2)
        %i_minus_two_v = lz.primop_sub(%i, %lit_two)
        %fib_i_minus_two_t = lz.ap(%fib_rec: !lz.fn<(!lz.value) -> !lz.value>, %i_minus_two_v)
  ...
  lz.func @main () -> !lz.value {
      %lit_6 = lz.make_i64(6)
      %fib = lz.ref(@fibstrict)  : !lz.fn<(!lz.value) -> !lz.value>
      %out_v = lz.ap(%fib : !lz.fn<(!lz.value) -> !lz.value>, %lit_6)
